    ...
    <div className="my-custom-content3" {...others}>
      <Box direction="row">
        <Menu
          ...
          popupClassName={styles.my_hoz_menu}
          popupAutoWidth
          popupAlign="outside"
          triggerType="hover"
        >
          {dataSource.map((item, i) => {
            if (item.children) {
              return (
                <SubMenu style={{ fontSize: 50 }} label={item.title} className={styles.navmenu_item}>
                  {
            ...
            return (
              // <Item key={item.id}><a href={item.link} className={styles.navmenu_item1}>{item.title}</a></Item>
              <Item className={styles.item1}><a href={item.link} className={styles.navmenu_item1}>{item.title}</a></Item>
            );
          })}
        </Menu>
      </Box>
