...
class ConfigureRepository
{
    ...
    private $modeltable;
    private $redisClient;
    public function __construct(Configure $configure, RedisClient $redisClient)
    {
    ...
    {
        $configure = $this->redisClient->hGetAll("configure");
        if (empty($shopConfig)) {
            $col = checkColumns($columns);
            if (empty($field)) {
                $configure = $this->model::select($col)->get()->toArray();
            } else {
                $where = str2arr($field);
                $configure = $this->model::select($col)->where($where)->get()->toArray();
            }
            $this->redisClient->hMset("configure", $configure);
        }
        return $configure;
    }
