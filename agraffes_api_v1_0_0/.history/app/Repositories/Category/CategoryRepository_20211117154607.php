...
class CategoryRepository
{
    ...
    public function getParent($cat_link)
    {
        $parent = $this->model::select("cat_link", "parent_id")->where("cat_link", "=", $cat_link)->value("parent_id");
        if ($parent === 0) {
            // $category['cat_one'] = $parent_id;
            // $category['cat_two'] = $this->model::where("parent_id", "=", $parent_id)->value("parent_id");
            // $category['cat_three'] = $parent_id;
        } else {
        }
        return $category;
    }
