...
class CategoryRepository
{
    ...
    public function allCategory($sort)
    {
    ...
    // private function getTree($tree_id, $top = 0)
    // {
    //     $three_arr = [];
    //     $count = Category::where('parent_id', $tree_id)->where('is_show', 1)->count();
    //     if (0 < $count || $tree_id == 0) {
    //         $res = Category::select('cat_id', 'cat_name', 'cat_icon', 'parent_id', 'is_show')->where('parent_id', $tree_id)->where('is_show', 1)->orderBy('sort_order', 'ASC')->orderBy('cat_id', 'ASC')->get()->toArray();
    ...
    //                 $three_arr[$k]['cat_img'] = $row['cat_icon'];
    ...
    private function custongetTree($tree_id, $top = 0)
    {
        ...
        if (0 < $count || $tree_id == 0) {
            $res = Category::select('cat_id as id', 'cat_name', 'cat_icon', 'parent_id', 'sort_order', 'is_show', 'show_in_nav', 'is_top_show')->where('parent_id', $tree_id)->orderBy('sort_order', 'ASC')->orderBy('cat_id', 'ASC')->get()->toArray();
            // return $res;
    ...
    //     $count = Category::where('parent_id', $tree_id)->where('is_show', 1)->count();
    //     if (0 < $count || $tree_id == 0) {
    //         $res = Category::select('cat_id as id', 'cat_name', 'cat_icon', 'parent_id', 'sort_order', 'is_show', 'show_in_nav', 'is_top_show')->where('parent_id', $tree_id)->where('is_show', 1)->orderBy('sort_order', 'ASC')->orderBy('cat_id', 'ASC')->get()->toArray();
    //         // return $res;
    ...
    {
        $goods = Goods::select('goods_id', 'goods_sn', 'goods_name')->where('cat_id', $id)->get()->toArray();
        return $goods;
    }
