...
class CategoryRepository
{
    ...
    private function custongetTree($tree_id, $top = 0)
    {
        ...
        if (0 < $count || $tree_id == 0) {
            $res = Category::select('cat_id', 'cat_name', 'cat_icon', "cat_link", 'parent_id', 'sort_order', 'is_show', 'show_in_nav', 'is_top_show')->where('parent_id', $tree_id)->orderBy('sort_order', 'ASC')->orderBy('cat_id', 'ASC')->get()->toArray();
            // return $res;

            foreach ($res as $k => $row) {
                $three_arr[$k]['cat_id'] = $row['id'];
                ...
                if (isset($row['cat_id'])) {
                    $child_tree = $this->custongetTree($row['cat_id']);
                    if ($child_tree) {
